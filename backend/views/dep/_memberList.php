...
$tableJs = <<<JS
    layui.use('table',function() {
        ...
        var renderOpt = {
            ...
            method:"POST", //
            where:{_csrfBackend:'$_csrfBackend',type:"$type",id:"$depId",t:"$t"},
            page: true, //开启分页
            limit: $pageSize,
            limits: $limitsJson,
            cols: [[
                    ...
                    case 'batchAdd':
                        ...
                        layer.confirm('确认要批量移入员工吗？',{icon: 3, title:'提示'},function(index){
                            Cajax({
                                type:"POST",
                                url:"$add",
                                data:{_csrfBackend:'$_csrfBackend',userId:userIds,id:"$depId",type:"$type"}
                    ...
                    case 'batchDel':
                        ...
                        layer.confirm('确认要批量移出员工吗？',{icon: 3, title:'提示'},function(index){
                            Cajax({
                                type:"POST",
                                url:"$add",
                                data:{_csrfBackend:'$_csrfBackend',userId:userIds,id:"$depId",type:"$type",t:"$t"}
                            },function () {},function(JsonData){
                                layer.msg(JsonData.msg,{icon:(JsonData.status) ? icon.ICON_OK : icon.ICON_ERROR});
                                if(JsonData.status){
                                    setTimeout("xadmin.close();", 3000 )
            ...
            layer.confirm('确认要'+txt+'该员工吗？',{icon: 3, title:'提示'},function(index){
                ...
                if(obj.event === 'add'){
                    Cajax({
                        type:"POST",
                        url:"$add",
                        data:{_csrfBackend:'$_csrfBackend',userId:userId,id:"$depId",type:"$type"}
                    },function () {},function(JsonData){
                        layer.msg(JsonData.msg,{icon:(JsonData.status) ? icon.ICON_OK : icon.ICON_ERROR});
                        if(JsonData.status){
                            setTimeout("xadmin.close();", 3000 )
                    ...
                    Cajax({
                        type:"POST",
                        url:"$add",
                        data:{_csrfBackend:'$_csrfBackend',userId:userId,id:"$depId",type:"$type",t:"$t"}
                    },function () {},function(JsonData){
                        layer.msg(JsonData.msg,{icon:(JsonData.status) ? icon.ICON_OK : icon.ICON_ERROR});
                        if(JsonData.status){
                            setTimeout("xadmin.close();", 3000 )
