;!function (win) {
  ...
  Xadmin.prototype.del_tab = function (id) {
     ...
     * @param  {Boolean} m  [是否全屏]
	 * @return {[type]}        [description]
	 */
	Xadmin.prototype.open = function (title,url,w,h,full,m) {
		if (title == null || title == '') {
	        var title=false;
	    };
	    if (url == null || url == '') {
...
layui.use(['layer','element','jquery'],function() {
    ...
    $('.left-nav #nav').on('click', 'li', function(event) {
        ...
        if($('.left-nav').css('width')=='60px'){
          $('.left-nav').animate({width: '60px'}, 100);//220px
          $('.page-content').animate({left: '60px'}, 100);//220px
          $('.left-nav i').css('font-size','14px');
          $('.left-nav cite,.left-nav .nav_right').show();
        }

        if($(window).width()<768){
            $('.page-content-bg').hide();//show
        }
    ...
    $('.left-nav #nav').on('mouseenter', '.left-nav-li', function(event) {
      if($('.left-nav').css('width')!='60px'){//220px
