                ...
                <div class="layui-card-body ">
                    <div class="demoTable" style="margin: 1px 0;margin-left: 0.5%;">
                        ...
                        <div class="layui-inline" style="margin-right: 10px;">
                            <input class="layui-input" name="parames" id="parames" autocomplete="off" placeholder="应用名或控制器名">
                        </div>
...
layui.use(['table','form'], function(){
    ...
    elem: '#demo'
    ,toolbar: '#toolbarDemo'
    ,url: "<?=\yii\helpers\Url::toRoute('config-page-manage/index') ?>" //数据接口
    ,page: true //开启分页
    ,id:'testReload'
    ,cols: [[ //表头
       {type: 'checkbox', fixed: 'left'}
      ,{field: 'page_name', title: '分页变量名',sort: true, fixed: 'left',templet:function (d) {
                        return '<p style="cursor:pointer;" title="分页配置ID:'+d.id+'">'+d.page_name+'</p>'}}
      ,{field: 'title', title: '所属应用'}
      ,{field: 'controller', title: '所属控制器'}
      ,{field: 'action', title: '所属方法'} 
      ,{field: 'num', title: '每页数量', edit: 'text'} 
      ,{field: 'show_name', title: '分页名称', sort: true} 
      ,{field: 'type', title: '分页类型', sort: true,templet:function(d){
         if(d.type == 1){
            return '<div>前台</div>'
         }else if(d.type == 2){
            return '<div>后台</div>' 
    ...
    table.on('edit(test)', function(obj){
        ...
        layer.confirm('确定修改吗？', function(index){
            ...
            $.get("<?=\yii\helpers\Url::toRoute('config-page-manage/update-num') ?>",{id:id,num:num},function(res){
                layer.close(index);
                if(res.code==200){
                    layer.msg('成功', {
                            time: 2000,//3s后自动关闭
  ...
  form.on('switch(status)', function(obj){
    ...
    var index = layer.load('修改中',1, {shade: false, offset: '300px'});
    $.post("<?=\yii\helpers\Url::toRoute('config-page-manage/update-status') ?>",{id:id,status:status},function(res){
        layer.close(index);
        if(res.code==200){
            layer.msg('修改成功', {
                time: 2000,//3s后自动关闭
  ...
  var $ = layui.$, active = {
     ...
     table.on('toolbar(test)', function(obj){
        ...
        case 'getCheckData':
            ...
            layer.confirm('确定批量删除吗？如果有配置为使用状态则会自动跳过删除。',{offset: 't'}, function(index){
                    ...
                    $.get("<?=\yii\helpers\Url::toRoute('config-page-manage/delete-all') ?>",{id:str},function(res){
                    layer.close(index);
                    if(res.code==200){
                        layer.msg('删除成功', {
                            time: 2000,//3s后自动关闭
    ...
    table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){ //删除
            ...
            layer.confirm('确定删除'+data.page_name+'么',{offset: 't'}, function(index){
                var index = layer.load('删除中',1, {shade: false, offset: '300px'});
                $.post("<?=\yii\helpers\Url::toRoute('config-page-manage/delete') ?>",send,function(res){
                 layer.close(index);
                if(res.code==200){
                    layer.msg('删除成功', {
                         time: 2000,//3s后自动关闭
            ...
            layer.open({
                type: 2,
                offset: 't',
                content:"<?=\yii\helpers\Url::toRoute('config-page-manage/update') ?>?id="+id,
                area:['50%','50%'],
                title:'修改'
            });
        }
    ...
    $(document).on('click','#create',function(){
        layer.open({
            type: 2,
            offset: 't',
            content:"<?=\yii\helpers\Url::toRoute('config-page-manage/create') ?>",
            area:['60%','80%'],
            title:'添加'
        });
    })
