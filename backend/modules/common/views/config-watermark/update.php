<div class="panel panel-default">
    <div class="panel-body">
        ...
        <input type="hidden" name="id" value = "<?=$data->id?>">
            ...
            <?php if($data->type == 1){ ?>
                <div class="layui-form-item" id="type-1">
            <?php }else{ ?>
            ...
            <?php } ?>
                <div class="layui-inline">
                    ...
                    <div class="layui-input-inline">
                        ...
                        <input type="hidden" name="watermark_url" id="watermark_url" value = "<?=$data->watermark_url?>" lay-verify="">
                    </div>
                    <div class="layui-input-inline">
                        <img style="max-width: 50px;max-height:50px;" id="img_url" value = "<?= Yii::$app->urlManager->baseUrl.'/'.$data->watermark_url?>">
                    </div>
                </div>
            </div>
            <?php if($data->type == 2){ ?>
                <div class="layui-form-item" id="type-2">
            <?php }else{ ?>
            ...
            <?php } ?>
               <div class="layui-inline">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="watermark_text" id="watermark_text" lay-verify=""  autocomplete="off" placeholder="请输入文字水印内容" class="layui-input" value = "<?=$data->watermark_text?>" >
                    </div>
                </div>
                <div class="layui-inline">
                    <label class="layui-form-label"><span class="x-red">*</span>文字大小</label>
                    <div class="layui-input-block">
                        <input type="text" name="text_size" id="text_size" lay-verify=""  autocomplete="off" placeholder="请输入文字大小" class="layui-input" value = "<?=$data->text_size?>" >
                    </div>
                </div>
            </div>
            <div class="layui-form-item">
               <div class="layui-inline">
                    <label class="layui-form-label"><span class="x-red">*</span>坐标X</label>
                        <div class="layui-input-inline">
                            <input type="text" name="x" lay-verify="required"  autocomplete="off" placeholder="坐标X" class="layui-input" value = "<?=$data->x?>"> 
                        </div>
                </div>
                <div class="layui-inline">
                    <label class="layui-form-label"><span class="x-red">*</span>坐标Y</label>
                        <div class="layui-input-inline">
                             <input type="text" name="y" lay-verify="required"  autocomplete="off" placeholder="坐标Y" class="layui-input" value = "<?=$data->y?>">
                        </div>
                </div>
            </div>
            <div class="layui-form-item">
...
<script type="text/javascript">
    ...
    layui.use(['form','upload'], function(){
        ...
        form.on('submit(add)', function(data){
            ...
            $.post("<?=\yii\helpers\Url::toRoute('config-watermark/update') ?>",data.field,function(res){
                if(res.code==200){
                        layer.msg('添加成功', {
                                time: 2000,//3s后自动关闭
                            },function(){
