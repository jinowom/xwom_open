...
layui.use(['table','form'], function(){
    ...
    elem: '#demo'
    ,toolbar: '#toolbarDemo'
    ,url: "<?=\yii\helpers\Url::toRoute('config-watermark/index') ?>" //数据接口
    ,page: true //开启分页
    ,id:'testReload'
    ,cols: [[ //表头
      ...
      ,{field: 'watermark_url', title: '水印图片路径', templet:function(d){
        return '<div class="layer-photos-demo" >'+
                    '<img layer-pid="" style="width:35px;" src="'+'<?=Yii::$app->urlManager->baseUrl?>/'+d.watermark_url+'" >'+
                '</div>';  
      }}
      ,{field: 'watermark_text', title: '文字水印内容'}
      ,{field: 'text_size', title: '文字大小'}
      ,{field: 'x', title: 'x坐标'}
      ,{field: 'y', title: 'y坐标'}
      ,{field: 'description', title: '操作说明'}
      ,{field: 'type', title: '水印类型', sort: true, templet:function(d) {
         if(d.type == 1){
            return '<div>图片水印</div>'
         }else if(d.type == 2){
            return '<div>文字水印</div>' 
      ...
      ,{field:'status', title:'是否使用', templet: '#switchTpl', unresize: true, width:110}
  ...
  form.on('switch(status)', function(obj){
    ...
    $.post("<?=\yii\helpers\Url::toRoute('config-watermark/update-status') ?>",{id:id,status:status},function(res){
        layer.close(index);
        if(res.code==200){
            layer.msg('修改成功', {
                time: 2000,//3s后自动关闭
  ...
  var $ = layui.$, active = {
    ...
    table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){ //删除
            ...
            layer.confirm('确定删除'+data.name+'么',{offset: 't'}, function(index){
                ...
                $.post("<?=\yii\helpers\Url::toRoute('config-watermark/delete') ?>", send,function(res){
                 layer.close(index);
                if(res.code==200){
                    layer.msg('删除成功', {
                         time: 2000,//3s后自动关闭
            ...
            layer.open({
                type: 2,
                offset: 't',
                content: "<?=\yii\helpers\Url::toRoute('config-watermark/update') ?>?id="+id,
                area:['60%','80%'],
                title:'修改'
            });
        }
    ...
    $(document).on('click','#create',function(){
        layer.open({
            type: 2,
            offset: 't',
            content: "<?=\yii\helpers\Url::toRoute('config-watermark/create') ?>",
            area:['60%','80%'],
            title:'添加'
        });
    })
