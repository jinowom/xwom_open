        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-body ">
                    <table class="layui-table" id="interfaceList" lay-filter="interfaceList"></table>
                </div>
            </div>
        </div>
    </div>
...
<input type="hidden" value="<?php echo $interfaceName;   ?>" id="interfaceName"/>
<script type="text/html" id="toolbar">
    <div class="layui-btn-container">
        <div class="layui-row layui-col-space10">
            <?php if($isSuper){  ?>
        <div class="layui-col-xs3 layui-col-sm2 layui-col-md1">
           <button class="layui-btn" onclick="xadmin.open('添加第三方接口','<?=\yii\helpers\Url::toRoute(['third-pary-interface/interface-edit'])?>',500,500)"><i class="layui-icon"></i>添加</button>
        </div>
            <?php  }  ?>
        <div class="layui-col-xs6 layui-col-sm6 layui-col-md6">
                <form class="layui-form layui-row layui-col-space5">
                    <div class="layui-col-xs8 layui-col-sm8 layui-col-md10">
                        <input type="text" name="interfaceName" id="interfaceName" placeholder="请输入接口名称" value="<?php echo $interfaceName;   ?>" autocomplete="off" class="layui-input"></div>
                    <div class="layui-col-xs4 layui-col-sm4 layui-col-md2"><button class="layui-btn" lay-submit="" lay-filter="sreach" id="searchInterface"><i class="layui-icon">&#xe615;</i></button></div>
                </form>
...
<?php
$getInterfaceList = \yii\helpers\Url::toRoute(['get-interface-list']);
...
$tableJs = <<<JS
    ...
    layui.use('table',function() {
            ...
            var renderOpt = {
                ...
                method:"POST", //
                where:{_csrfBackend:'$_csrfBackend',interfaceName:interfaceName},
                page: true, //开启分页
                limit: $pageSize,
                limits: $limitsJson,
                cols: [[
                    {field: 'id', checkbox:true,title: 'ID', width:100, sort: true, fixed: 'left'},
                    {field: 'name', title: '接口名称', minWidth:100, sort: true},
                    {field: 'type', title: '官方', minWidth:100, sort: true},
                    {field: 'unitId', title: '所属单位', minWidth:100, sort: true},
            ...
            //头工具栏事件
            table.on('toolbar(interfaceList)',
                function(obj) {
            ...
            //监听行工具事件
            table.on('tool(interfaceList)', function(obj){
                var data = obj.data;
                if(obj.event === 'del'){
                    rowDel(obj,data);
            ...
            $(function(){
               $('body').on('click','#searchInterface',function(){
