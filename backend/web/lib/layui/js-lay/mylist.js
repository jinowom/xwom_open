...
layui.define(['form','layer','table','laydate'],function(exports){ //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
    var layer=layui.layer,table = layui.table, $ = layui.jquery,laydate = layui.laydate,addUrl,auditUrl,form = layui.form,editUrl,delUrl,csrfToken,page,pages;
    var T = {
        init:function(ele){
            var $ele = $("#"+ele);
        ...
        render:function(ele){
            ...
            table.render({
                ...
                cols: [[
                    ...
                    {field: 'status',align:'center', title: '稿件状态', minWidth:120,width:150, sort: true,},
                    {field: 'log',align:'center', title: '记录', templet: '#checkboxTpl',minWidth:120,width:150, sort: true,},
        ...
        toolTable:function(ele){
            ...
            table.on('tool('+ele+')',function(obj){
                ...
                if(obj.event === 'del'){
                    ...
                    _this.approve(obj,ur);
                } else if(obj.event === 'auditLog'){
                    xadmin.open('审核记录',auditUrl+"&id="+data.id,'500','500',false,false);
                }
            });
        },
        approve: function(obj,url){
            layer.confirm('确定要投递该稿件吗？', function(index){
                Cajax({
                    url:url,data:{id:obj.data.id},type:'get'
                },function(){
                    layer.closeAll();
                    layer.load(2);
                },function(JsonData){
